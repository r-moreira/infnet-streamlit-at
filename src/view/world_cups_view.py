...
class WorldCupsViewOption(Enum):
    TEAM = "Team"
    MATCH = "Match"
    PLAYER = "Player"
    
    def to_list() -> List:
        return [e.value for e in WorldCupsViewOption]
...
class WorldCupsView(AbstractViewStrategy):
    ...
    class State:
        @staticmethod
        def set_view_menu_option(menu_option: WorldCupsViewOption | None) -> None:
            if 'world_cup_view_menu_option' not in st.session_state or menu_option is None:
                st.session_state['world_cups_view_menu_option'] = WorldCupsViewOption.TEAM
            st.session_state['world_cups_view_menu_option'] = st.session_state['world_cups_view_menu_option']
            
        @staticmethod
        def get_view_menu_option() -> WorldCupsViewOption:
            if 'world_cups_view_menu_option' not in st.session_state:
                WorldCupsView.State.set_view_menu_option(WorldCupsViewOption.TEAM)
    ...
    def render(self) -> None:
        ...
        if menu_option == WorldCupsViewOption.TEAM:
            self.team_fragment()
        elif menu_option == WorldCupsViewOption.MATCH:
            self.match_fragment(team_matches)
        elif menu_option == WorldCupsViewOption.PLAYER:
    ...
    def option_menu_fragment(self):
        ...
        if 'world_cups_view_menu_option' in st.session_state:
            menu_index = WorldCupsViewOption.to_list().index(st.session_state['world_cups_view_menu_option'])

        menu_option = option_menu(
            None, 
            options=WorldCupsViewOption.to_list(), 
